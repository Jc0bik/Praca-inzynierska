...
namespace InzV3.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var user = await UserManager.FindByNameAsync(model.LoginIdentifier) ?? await UserManager.FindByEmailAsync(model.LoginIdentifier);
            if (user == null)
            {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    TempData["SuccessMessage"] = $"Użytkownik {model.FirstName} {model.LastName} został pomyślnie zarejestrowany.";
